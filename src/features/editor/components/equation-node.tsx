...
/* oxlint-disable jsx-a11y/no-autofocus -- the TeX input auto-focuses when the equation popover opens. */
            ...
            >
              {props.element.texExpression.length > 0 ? (
                ...
                <div className="flex h-7 w-full items-center gap-2 whitespace-nowrap text-muted-foreground text-sm">
                  ...
                  <div>Aggiungi un'equazione TeX</div>
                </div>
              )}
            </div>
          }
...
export function InlineEquationElement(props: PlateElementProps<TEquationElement>) {
  ...
  React.useEffect(() => {
    if (selected && isCollapsed) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- open the inline equation popover on selection.
      setOpen(true);
    }
  }, [selected, isCollapsed]);

  useEquationElement({ element, katexRef, options: katexOptions });
  ...
  return (
    <PopoverContent className="flex gap-2" contentEditable={false}>
      ...
      <Button className="px-3" onClick={onClose} variant="secondary">
        Fatto <CornerDownLeftIcon className="size-3.5" />
      </Button>
    </PopoverContent>
  );
};
