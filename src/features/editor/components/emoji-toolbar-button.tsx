...
/* oxlint-disable jsx-a11y/no-autofocus -- the emoji search input auto-focuses when the picker opens. */
...
function EmojiPickerSearchAndClear({
  ...
  return (
    <div className="flex items-center text-foreground">
      <div className="absolute top-1/2 left-2.5 z-10 flex size-5 -translate-y-1/2 items-center justify-center text-foreground">
        {emojiSearchIcons.loupe}
      </div>
      {searchValue && (
        <Button
          aria-label="Cancella"
          className="absolute top-1/2 right-0.5 flex size-8 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border-none bg-transparent text-popover-foreground hover:bg-transparent"
          onClick={clearSearch}
          size="icon"
          title={i18n.clear}
          type="button"
...
> = {
  ...
  flags: { outline: <FlagIcon className="size-full" />, solid: <FlagIcon className="size-full" /> },
  foods: {
